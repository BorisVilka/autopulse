package ru.autopulse05.android.feature.company.presentation.conditions
...
fun CompanyConditionsScreen() {
  ...
  Column(
    ...
    MarkedListItem(text = PresentationText.Dynamic("Если ЗАДОЛЖЕННОСТЬ за товар превышает 16% от ОБОРОТА ЗАКУПОК, в независимости от условий размещения заказов, прием заказов ПРИОСТАНАВЛИВАЕТСЯ;"))
    MarkedListItem(text = PresentationText.Dynamic("ОТГРУЗКА товара происходит в пределах ПОЛОЖИТЕЛЬНОГО баланса."))

    Spacer(modifier = Modifier.height(SpaceExtraSmall))

    Text(
      text = PresentationText.Dynamic("При обороте закупок свыше 1 000 000 рублей возможны ОСОБЫЕ условия заказов и отгрузки. Предоставляется индивидуально.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("По всем вопросам предоставления СКИДОК и УСЛОВИЙ работы:").asString(),
    ...
    Text(
      text = PresentationText.Dynamic("+7 922-39-81-555  8 (928) 049-25-04  Звонок бесплатный.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("[Авто пульс 05], действующий на основании свидетельства о регистрации № 558844, именуемый в дальнейшем «Продавец» и гражданин [CLIENT], именуемый в дальнейшем «Заказчик», заключили договор о нижеследующем:").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("1. Заказчик заказывает,  а Продавец обязуется доставить авто запчасти (далее Товар) согласно приложению №1 к настоящему договору.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("3. Продавец вправе в одностороннем порядке отказаться от поставки Товара (если, по независящим от Продавца обстоятельствам, заказанный Товар не может быть доставлен), в таком случае Продавец обязан в кратчайший срок  информировать об этом Заказчика и вернуть ему предоплату.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("4. Передача  Товара от  Продавца к Заказчику осуществляется только после полной оплаты стоимости Товара Заказчиком.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("5.Факт приемки товара по качеству и количеству удостоверяется  подписью Заказчика.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("6. Заказчик обязан предоставить продавцу достоверные данные об автомобиле (идентификационный номер автомобиля VIN), для которого приобретается соответствующая запасная часть (Товар). В противном случае Продавец не несет никакой ответственности за соответствие Товара требованиям, которые к нему предъявляет Заказчик.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("8. В случае ошибки, допущенной продавцом, при определении необходимой Заказчику детали, ее обмен осуществляется при условии, если указанная деталь не была в употреблении, сохранены товарный вид детали и ее упаковки. Обмен производится в течение 7 календарных дней, при наличии кассового чека.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("11. Полученный по заказу Товар хранится на складе Продавца в течение 30 дней, затем реализуется . Возврат предоплаты Заказчику осуществляется после продажи Товара.").asString(),
      style = MaterialTheme.typography.body1
    )
    ...
    Text(
      text = PresentationText.Dynamic("12.Сроки доставки заказа действует от 1 до 4 рабочих дней.").asString(),
      style = MaterialTheme.typography.body1
    )
...
@Preview(
  name = "Company Conditions Screen",
  showBackground = true,
  uiMode = Configuration.UI_MODE_NIGHT_YES
)
@Composable
fun CompanyConditionsScreenPreview() {
